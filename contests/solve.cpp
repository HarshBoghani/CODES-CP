...
ll solve(int i, int maxi, int flag, string &s,vector<vector<vector<int>>>& dp){
    if(i==-1){
        return 0;
    }
    if(dp[i][maxi][flag]!=INT_MAX) return dp[i][maxi][flag];
    if(flag==0){
        ll take=INT_MIN;
        for(int x=0;x<5;x++){
            if(x>=maxi){
                take = max(take,ten[x]+solve(i-1,x,1,s,dp));
            }else{
                take = max(take,-ten[x]+solve(i-1,maxi,1,s,dp));
            }
        }
        ll ntake = INT_MIN;
        int x = s[i]-'A';
        if(x>=maxi){
            ntake = max(ntake,ten[x]+solve(i-1,x,0,s,dp));
        }else{
            ntake = max(ntake,-ten[x]+solve(i-1,maxi,0,s,dp));
        }
        return dp[i][maxi][flag]=max(take,ntake);
    }else{
        ll ntake = INT_MIN;
        int x = s[i]-'A';
        if(x>=maxi){
            ntake = max(ntake,ten[x]+solve(i-1,x,1,s,dp));
        }else{
            ntake = max(ntake,-ten[x]+solve(i-1,maxi,1,s,dp));
        }
        return dp[i][maxi][flag]=ntake;
    }
    return -1;
}
int main() {
    ...
    while (t--) {
        ...
        cin>>s;
        int n = s.size();
        vector<vector<vector<int>>> dp(n,vector<vector<int>>(5,vector<int>(2,INT_MAX)));
        cout<<solve(n-1,0,0,s,dp)<<endl;
    }
    return 0;
}
